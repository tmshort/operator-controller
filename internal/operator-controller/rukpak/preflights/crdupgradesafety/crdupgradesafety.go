...
func (p *Preflight) InstallObjects(ctx context.Context, objs []client.Object) error {
	return p.runPreflightObjects(ctx, objs)
}
...
func (p *Preflight) UpgradeObjects(ctx context.Context, objs []client.Object) error {
	return p.runPreflightObjects(ctx, objs)
}
...
func (p *Preflight) runPreflight(ctx context.Context, rel *release.Release) error {
	...
	return p.runPreflightObjects(ctx, relObjects)
}

func (p *Preflight) runPreflightObjects(ctx context.Context, relObjects []client.Object) error {
	if len(relObjects) == 0 {
		return nil
	}
